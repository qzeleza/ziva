...
description: "Battle-tested guidelines for building TUIs with Termos"
---

# Termos Best Practices

Use this checklist when integrating Termos into production systems. It covers structure, validation, styling, performance and testing.
...
- Call `termos.AutoConfigure()` plus language setup (`SetDefaultLanguage`, `SetLanguage`) in `main()` before you start creating queues.
...
- Keep `Description()` messages user-friendly — Termos renders them under the field.
...
- Customise error colours with `termos.SetErrorColor` to match brand requirements; call `ResetErrorColors()` after experiments.
...
- Limit completed-task history with the `TERMOS_MAX_COMPLETED_TASKS` environment variable.
- Monitor memory via `TERMOS_MEMORY_PRESSURE_THRESHOLD` and periodically purge the string intern cache using `termos.ClearInternCache()` for long-lived CLIs.
...
- Log critical failures outside of Termos (e.g. `log`/`zap`), especially when queues halt due to `WithStopOnError(true)`.
...
Pair these guidelines with the examples section to build reliable and friendly TUIs with Termos.
