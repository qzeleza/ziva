...
description: "All task types in Termos and their key capabilities"
---

# Task Types in Termos

Termos provides five main task types for creating interactive TUI applications. Each task has unique capabilities and use cases.
...
All tasks in Termos share a common set of basic capabilities:
...
```go
// YesNo task
yesno := termos.NewYesNoTask("Title", "Question?")

// Single select task
single := termos.NewSingleSelectTask("Title", []string{"opt1", "opt2"})

// Multi select task
multi := termos.NewMultiSelectTask("Title", []string{"item1", "item2"})

// Input task
input := termos.NewInputTask("Title", "Prompt:")

// Function task
fn := termos.NewFuncTask("Title", func() error {
    // perform work
    return nil
})
```
...
```go
task := termos.NewSingleSelectTask("Selection", options).
