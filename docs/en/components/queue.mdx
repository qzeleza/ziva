---
title: "Task Queue"
description: "Task sequence management system in Termos"
---

# Task Queue

`Queue` is the central component of Termos that manages sequential task execution and provides a unified interface for user interaction.

## Creating a Queue

```go
queue := termos.NewQueue("Queue title")
```
...
queue.WithAppNameColor(termos.BlueBright, true) // color, bold

// Queue title color
queue.WithTitleColor(termos.GreenBright, false) // color, normal
```
...
```go
queue := termos.NewQueue("Simple application")
...
```go
queue := termos.NewQueue("Advanced TUI Application")
