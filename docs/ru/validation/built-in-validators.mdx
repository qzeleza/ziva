...
Termos поставляется с фабрикой валидаторов `termos.DefaultValidators`. Все методы возвращают готовый объект, который можно передать в `InputTask.WithValidator()`.

```go
v := termos.DefaultValidators
input := termos.NewInputTask("Email", "Укажите адрес:").
...
login := termos.NewInputTask("Логин", "Введите имя:").
    WithValidator(v.Username())

// Опциональный email
supportEmail := termos.NewInputTask("Доп. email", "Можно пропустить")
   .WithAllowEmpty(true)
...
value := termos.NewInputTask("Сервисный ID", "Введите код:")
