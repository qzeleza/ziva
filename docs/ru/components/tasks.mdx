---
title: "Обзор задач"
description: "Какие типы задач доступны в Termos и как с ними работать"
---

# Типы задач Termos

Termos предоставляет пять готовых типов задач. Каждая из них решает конкретный сценарий пользовательского ввода и интегрируется в очередь без ручного управления состоянием.
...
`termos.Task` — это алиас интерфейса `common.Task`. Если встроенных типов недостаточно, можно реализовать его самостоятельно:
...
func (t *customTask) Update(msg tea.Msg) (termos.Task, tea.Cmd) { return t, nil }
...
func (t *customTask) WithNewLinesInErrors(bool) termos.Task { return t }
```
...
```go
queue := termos.NewQueue("Настройка кластера")

confirm := termos.NewYesNoTask("Подтверждение", "Продолжить?")
mode    := termos.NewSingleSelectTask("Режим", modes)
features := termos.NewMultiSelectTask("Фичи", featureList)
name     := termos.NewInputTask("Имя", "Введите имя кластера")
build    := termos.NewFuncTask("Сборка", assembleCluster)
...
- **Не дублируйте вывод.** Termos сам управляет TUI-рендерингом; избегайте `fmt.Println` внутри задач.
