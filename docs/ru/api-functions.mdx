...
```go
// Сделать «Выход» визуально таким же, как остальные строки меню
ziva.SetExitMenuItemStyle(ziva.MenuItemDefaultStyle())

// Настроить собственный вариант подсветки
...
Меняет стиль отображения локализованных пунктов меню «Назад». По умолчанию используется тот же подсвеченный стиль, что и для «Выход».

```go
// Сделать пункт «Назад» нейтральным
ziva.SetBackMenuItemStyle(ziva.MenuItemDefaultStyle())

// Вернуть стандартную подсветку позже
ziva.ResetBackMenuItemStyle()
```

### ResetExitMenuItemStyle()

Возвращает подсветку пункта «Выход» к значениям по умолчанию.
...
Возвращает подсветку пункта «Назад» к значениям по умолчанию.
...
Возвращает бирюзовый жирный стиль для специальных пунктов меню.

```go
multi.WithSelectAll("Включить всё", "Убрать всё", ziva.MenuActionDefaultStyle())
```

### MenuItemDefaultStyle() lipgloss.Style

Возвращает стиль, совпадающий с обычными пунктами меню (белый текст).

```go
multi.WithSelectAll("Включить всё", "Убрать всё", ziva.MenuItemDefaultStyle())
```
