...
description: "Простые примеры использования Termos для создания TUI приложений"
---

# Базовые примеры использования

Эта страница содержит практические примеры создания TUI приложений с помощью Termos. Все примеры готовы к запуску и демонстрируют основные возможности библиотеки.
...
func main() {
    ...
    queue := termos.NewQueue("Очистка системы")
...
func main() {
    ...
    queue := termos.NewQueue("Развертывание приложения")
...
func main() {
    ...
    queue := termos.NewQueue("Установка системы")
...
func main() {
    ...
    name := termos.NewInputTask("Имя", "Введите ваше имя:").
    ...
    website := termos.NewInputTask("Сайт", "URL сайта (опционально):").
...
func main() {
    ...
    // Настраиваем очередь
    queue := termos.NewQueue("Настройка веб-сервера")
...
func createConfigTasks(config *ServerConfig) []termos.Task {
    ...
    // 1. Хост
    hostTask := termos.NewInputTask("Хост сервера", "IP адрес или домен:").
        WithValidator(v.Required()).
        WithTimeout(30*time.Second, "localhost")

    // 2. Порт
    portTask := termos.NewInputTask("Порт", "Порт сервера (1-65535):").
        WithInputType(termos.InputTypeNumber).
        WithValidator(v.Port()).
        WithTimeout(15*time.Second, "8080")

    // 3. SSL
    sslTask := termos.NewYesNoTask("SSL", "Включить HTTPS?").
        WithDefaultItem("Да")
    ...
    moduleTask := termos.NewMultiSelectTask("Модули", modules).
...
authTasks := []termos.Task{
    termos.NewInputTask("Username", "Логин:"),
    termos.NewInputTask("Password", "Пароль:").WithInputType(termos.InputTypePassword),
}

// Группа настроек
configTasks := []termos.Task{
    termos.NewSingleSelectTask("Theme", themes),
    termos.NewYesNoTask("Notifications", "Включить уведомления?"),
}
