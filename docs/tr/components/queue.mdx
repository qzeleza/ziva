...
description: "Termos'ta görev dizilerinin yönetim sistemi"
---

# Görev Kuyruğu (Queue)

`Queue`, Termos'un merkezi bileşenidir. Görevlerin sıralı yürütülmesini yönetir ve kullanıcı etkileşimi için birleşik bir arayüz sağlar.

## Kuyruk Oluşturma

```go
queue := termos.NewQueue("Kuyruk başlığı")
```
...
```go
// Uygulama adı rengi
queue.WithAppNameColor(termos.BlueBright, true) // renk, kalın

// Kuyruk başlığı rengi
queue.WithTitleColor(termos.GreenBright, false) // renk, normal
```
...
```go
queue := termos.NewQueue("Basit uygulama")
...
```go
queue := termos.NewQueue("Gelişmiş TUI Uygulaması")
...
```go
queue := termos.NewQueue("IoT Yapılandırması")
