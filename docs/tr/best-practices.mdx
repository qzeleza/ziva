...
description: "Termos ile TUI geliştirmek için kanıtlanmış rehber"
---

# Termos En İyi Uygulamaları

Termos'u üretim sistemlerine entegre ederken bu rehberi kullanın. Yapı, doğrulama, stil, performans ve test konularında ipuçları içerir.
...
- Kuyruk oluşturmadan önce `main()` içinde `termos.AutoConfigure()` çağırın ve dil ayarlarını (`SetDefaultLanguage`, `SetLanguage`) yapın.

## Kuyruk yönetimi

- Uzun senaryolarda `WithTasksNumbered(true, "[%02d]")` kullanarak numaralandırma ekleyin; operatörler daha hızlı yön bulur.
- Son özet gerekmediğinde `WithOutSummary(true)` ile kapatın; aksi hâlde `WithSummaryFunction` ile önemli sonuçları satır içinde gösterin.
...
- `Description()` metinlerini kullanıcı dostu tutun — Termos metni alanın altında gösterir.
...
- Marka renklerini yakalamak için `termos.SetErrorColor` kullanın; denemelerden sonra `ResetErrorColors()` çağırmayı unutmayın.
- Kısıtlı terminallerde `EnableEmbeddedMode()` ve `EnableASCIIMode()` kombinasyonu çıktıyı okunur tutar.
- Birden fazla dilde açıklama sağlamak için `SetChoiceHelpDelimiter()` ile yardım metinleri ekleyin (`"prod::Canlı ortam"`).

## Performans ve gömülü hedefler

- Tamamlanan görev geçmişini `TERMOS_MAX_COMPLETED_TASKS` ortam değişkeniyle sınırlandırın.
- Belleği `TERMOS_MEMORY_PRESSURE_THRESHOLD` ile izleyin ve uzun ömürlü CLI'larda `termos.ClearInternCache()` çağırarak string önbelleğini temizleyin.
...
- `WithStopOnError(true)` nedeniyle durdurulan kuyruklar için Termos dışında (örn. `log`, `zap`) hata kaydı tutun.
...
Bu önerileri örnekler bölümündeki kodlarla birlikte kullanarak dayanıklı ve kullanıcı dostu Termos tabanlı TUI'ler geliştirin.
